...
export default function MobileBottomNav({ onAttendanceClick }) {
    ...
    const isEnrolled = Boolean(auth?.user?.is_enrolled);
    ...
    const isMasterProfile = route().current('student.profile.*');
    const isApply = route().current('student.application.*');
            ...
            >
                <div className="flex items-center justify-around max-w-md mx-auto">
                    ...
                    >
                        ...
                        <span className="text-[10px] mt-1 tracking-tight">Portal</span>
                    </Link>
                    ...
                    >
                        <div className="relative">
                            <FileText className={`h-5 w-5 ${isMasterProfile ? 'stroke-[2.5]' : 'stroke-2'}`} />
                            {isMasterProfile && (
                                <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 h-1 w-3 rounded-full bg-govt-green" />
                            )}
                        </div>
                    ...
                    >
                        <div className="relative">
                            <GraduationCap className={`h-5 w-5 ${isApply ? 'stroke-[2.5]' : 'stroke-2'}`} />
                            {isApply && (
                                <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 h-1 w-3 rounded-full bg-govt-green" />
                            )}
                        </div>
